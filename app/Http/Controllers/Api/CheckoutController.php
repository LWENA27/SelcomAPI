...
use AppModelsStoredCard;
use AppModelsTillAlias;
...
class CheckoutController extends Controller
{
    ...
    public function createOrderFull(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'vendor' => 'required|string|max:50',
            'order_id' => 'required|string|max:100',
            ...
            'buyer_name' => 'required|string|max:255',
            'buyer_phone' => 'required|string|max:20',
            'buyer_userid' => 'nullable|string|max:100',
        ...
        try {
            $order = Order::create([
                ...
                'payment_gateway_url' => $this->generateGatewayUrl($validated['order_id']),
                'qr_code' => $this->generateQRCode($this->generatePaymentToken()),
                'gateway_buyer_uuid' => Str::uuid()->toString(),
                'buyer_remarks' => $validated['buyer_remarks'] ?? null,
                'merchant_remarks' => $validated['merchant_remarks'] ?? null,
                'no_of_items' => $validated['no_of_items'] ?? 1,
                'webhook_url' => isset($validated['webhook_url']) ? base64_encode($validated['webhook_url']) : null,
                'redirect_url' => isset($validated['redirect_url']) ? base64_encode($validated['redirect_url']) : null,
                'cancel_url' => isset($validated['cancel_url']) ? base64_encode($validated['cancel_url']) : null,
                'expires_at' => now()->addMinutes($validated['expiry'] ?? 60),
            ]);

            Log::info('Full order created', ['vendor' => $order->vendor, 'order_id' => $order->order_id]);
            ...
            return $this->errorResponse('Order creation failed', '500', 'Internal server error');
        }
    }

    public function getStoredCards(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'vendor' => 'required|string|max:50',
            'buyer_userid' => 'required|string|max:100',
        ]);
    ...
    public function deleteCard(Request $request)
    {
        ...
        $card = StoredCard::where('vendor', $request->input('vendor'))
        ...
        Log::info('Card deleted', ['card_id' => $card->id, 'vendor' => $request->input('vendor')]);
    ...
    public function cardPayment(Request $request)
    {
        ...
        if ($request->input('save_card')) {
            StoredCard::firstOrCreate(
                ...
                [
                    'gateway_buyer_uuid' => $order->gateway_buyer_uuid,
                    'card_brand' => $request->input('card_brand', 'VISA'),
                    'last4_digits' => $request->input('last4', '****'),
        ...
        Log::info('Card payment processed', ['order_id' => $order->order_id, 'transid' => $transid]);
    ...
    public function walletPayment(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'vendor' => 'required|string|max:50',
            'order_id' => 'required|string|max:100',
            'channel' => 'required|string|in:MPESA-TZ,AIRTELMONEY,TIGOPESATZ,HALOPESATZ',
    ...
    public function selcomPesaPayment(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'vendor' => 'required|string|max:50',
            'order_id' => 'required|string|max:100',
        ...
        $transid = 'SPESA' . strtoupper(Str::random(8));
    ...
    public function createTillAlias(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'vendor' => 'required|string|max:50',
            'alias_name' => 'required|string|max:100|unique:till_aliases,alias_name',
        ...
        try {
            ...
            Log::info('Till alias created', ['alias_name' => $alias->alias_name, 'vendor' => $alias->vendor]);

            return response()->json([
                'reference' => Str::uuid()->toString(),
                'resultcode' => '000',
                'result' => 'SUCCESS',
                'message' => 'Till alias created successfully',
                'data' => $alias->toSelcomResponse(),
            ], 201);

        } catch (\Exception $e) {
            Log::error('Till alias creation failed', ['error' => $e->getMessage()]);
            return $this->errorResponse('Failed to create till alias', '500', 'Internal server error');
        }
    }
}
